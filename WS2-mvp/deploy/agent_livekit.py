...
# In deploy mode, everything is in the same directory
load_dotenv()

# Write GCP service account JSON to file for ADC (needed by langchain-google-genai)
_creds_json = os.getenv("GOOGLE_CREDENTIALS_JSON")
if _creds_json:
    _sa_path = Path("/tmp/gcp-sa.json")
    _sa_path.write_text(_creds_json)
...
SYSTEM_PROMPT = (
    Path(__file__).parent / "jio_home_assistant" / "prompts" / "system.md"
).read_text()
...
def _gcp_credentials_info():
    """Load GCP credentials from GOOGLE_CREDENTIALS_JSON env var (for cloud deploy)."""
...
def get_stt():
    ...
    if provider == "sarvam":
        ...
        creds = _gcp_credentials_info()
        return google.STT(credentials_info=creds) if creds else google.STT()
...
def get_tts():
    ...
    if provider == "sarvam":
        ...
        from livekit.plugins import google
        creds = _gcp_credentials_info()
        return google.TTS(credentials_info=creds) if creds else google.TTS()
